...
open class CardTemplateEditor : AnkiActivity(), DeckSelectionListener {
    ...
    fun configureToolbar(menu: Menu) {
        menu.clear()
        menuInflater.inflate(R.menu.card_template_editor, menu)
        if (currentFragment?.noteTypeCreatesDynamicNumberOfNotes() == true) {
            ...
            val template = currentFragment?.getCurrentTemplate()
        ...
        val isInsertFieldItemVisible = currentFragment?.currentEditorViewId != R.id.styling_edit
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            ...
            R.id.action_reposition -> currentFragment?.showRepositionDialog()
            R.id.action_rename -> currentFragment?.showRenameDialog()
            R.id.action_copy_as_markdown -> currentFragment?.copyMarkdownTemplateToClipboard()
            R.id.action_insert_field -> currentFragment?.showInsertFieldDialog()
            R.id.action_delete -> {
            ...
            R.id.action_confirm -> {
                ...
                if (modelHasChanged()) {
                    val confirmButton = findViewById<View>(R.id.action_confirm)
            ...
            R.id.action_card_browser_appearance -> {
                ...
                val currentTemplate = currentFragment?.getCurrentTemplate()
                currentTemplate?.let { currentFragment?.launchCardBrowserAppearance(it) }
                return true
            }
        }
    ...
    class CardTemplateFragment : Fragment() {
        ...
        fun noteTypeCreatesDynamicNumberOfNotes(): Boolean {
        ...
        fun launchCardBrowserAppearance(currentTemplate: JSONObject) {
        ...
        @CheckResult
        fun getCurrentTemplate(): JSONObject? {
        ...
        fun deletionWouldOrphanNote(col: Collection, tempModel: CardTemplateNotetype?, position: Int): Boolean {
         ...
         */
        fun confirmDeleteCards(tmpl: JSONObject, notetype: NotetypeJson, numAffectedCards: Int) {
         ...
         * @param numAffectedCards number of cards which will be affected
         */
        fun confirmAddCards(notetype: NotetypeJson, numAffectedCards: Int) {
            val d = ConfirmationDialog()
            val msg = String.format(
                resources.getQuantityString(
